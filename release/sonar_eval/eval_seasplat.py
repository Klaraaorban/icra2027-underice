"""Evaluate a trained SeaSplat checkpoint against the real altimeter trace.

SeaSplat has no sonar supervision -- this measures its raw geometry the
same way eval_ours.py scores our model, using the checkpoint's own
train/test split (no retraining involved).

Run with SeaSplat's own conda env, from inside the SeaSplat repo root
(its `scene` / `gaussian_renderer` / `arguments` modules share names with
the main gaussian-splatting repo, so cwd matters):

    cd /path/to/seasplat
    python /path/to/eval_seasplat.py --config configs/ps117_39.yaml \
        --model-path /path/to/checkpoint --iteration 7000 --out results_seasplat.json
"""
import argparse
import json
import sys
...
def render_frame_depths(model_path, iteration, valeport_path, clip_start_iso, fps, half_patch=10):
    import torch
    ...
    with torch.no_grad():
        for split, views in [("train", scene.getTrainCameras()), ("test", scene.getTestCameras())]:
            for view in views:
                alpha = render(view, gaussians, pp, bg)["alpha"]
                depth = render_depth(view, gaussians, pp, bg)["render"][0].unsqueeze(0) / alpha
                bad = torch.isnan(depth) | torch.isinf(depth)
                if bad.any():
                    fill = depth[~bad].max().item() if (~bad).any() else 100.0
                    depth = torch.where(bad, torch.full_like(depth, fill), depth)
...
def main():
    ...
    rows = render_frame_depths(args.model_path, args.iteration, cfg["valeport_path"], cfg["clip_start"],
    ...
    summary = {
        "model": "SeaSplat (no sonar supervision)",
